...
IMPLEMENT_DYNAMIC(CLed4TestDlg, CDialogEx)

CLed4TestDlg::CLed4TestDlg(LPVOID p, TESTERTYPE type, CWnd* pParent /*=NULL*/)
	: CDialogEx(CLed4TestDlg::IDD, pParent),
m_Result(MJ_NORMAL),
m_nTotalNum(0),
m_nPassNum(0),
m_nFailNum(0)
...
CLed4TestDlg::~CLed4TestDlg()
{
	CloseSerialPort(m_hDevice);
}

void CLed4TestDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CLed4TestDlg, CDialogEx)
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_BTN_START, &CLed4TestDlg::OnBnClickedBtnStart)
END_MESSAGE_MAP()
...
HBRUSH CLed4TestDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
BOOL CLed4TestDlg::OnInitDialog()
{
	...
	case TP_LED:
		{
			SetWindowTextA(this->m_hWnd, "Led-Test(fun)");
		}
		break;

	case TP_WIFI:
		{
			SetWindowTextA(this->m_hWnd, "Wifi-Test(fun)");
		}
		break;

	case TP_FUN_MB:
		{
			SetWindowTextA(this->m_hWnd, "MainBoard-Test(fun)");
		}
		break;
	}
...
VOID			CLed4TestDlg::RefreshDeviceListR()
{
	wstring		wstr;
	VECSTR		vs;
...
VOID			CLed4TestDlg::RefreshDeviceList()
{
	wstring		wstr;
	wstring		wstrTmp;
	VECSTR		vs;
...
unsigned WINAPI		_threadForCheck4(LPVOID p)
{
	CLed4TestDlg*	pThis=(CLed4TestDlg*)p;
		...
		case TP_FUN_MB:
			{
				...
				pThis->m_strJYFormatFileContent+="TesterDesc|主板(FUN)测试站\r\nTesterID|0002\r\n";
				pThis->m_strJYFormatFileContent+="PN|";
				pThis->m_strJYFormatFileContent+=pThis->m_strPSN;
				pThis->m_strJYFormatFileContent+="Line|0001\r\nOperatorNum|0002\r\nTestDate|";
				pThis->m_strJYFormatFileContent+=csBuf;
...
void CLed4TestDlg::OnBnClickedBtnStart()
{
	this->SetBtnStatus(FALSE);
	...
	if( this->CheckSN())
	{
		...
		CloseHandle((HANDLE)_beginthreadex(NULL, NULL, _threadForCheck4, this, NULL, NULL));
	}
	else
	{
...
VOID  CLed4TestDlg::ShowInfo(PWCHAR pInfo, BOOL	bNew)
{
	wstring		wstr;
	if( bNew)
	{
...
BOOL			CLed4TestDlg::UpdaterMB(string strWorkDir)
{
	BOOL		bRet=FALSE;
...
BOOL			CLed4TestDlg::MainBoardCheck()
{
	BOOL		bRet=FALSE;
	string		strFWPath;
	string		strWorkDir;
...
BOOL			CLed4TestDlg::MainBoardSportCheck()
{
	BOOL		bRet=FALSE;
	string		strFWPath;
	string		strWorkDir;
	...
	//获取升级状态时的超时
	const	DWORD			dwTimeOutV=10*1000;
	...
	do 
	{
		...
		if( P_GetCheckResult(m_hDevice, m_testType, retDatas, SHORTLEN))
		{
			...
			*/
			...
			nResult=*(PLONG)(&retDatas[1]);
			nResult=ntohl(nResult);
			CF_OutputDBGInfoEx(GDBGTYPE, NULL, __FUNCTION__"...MB(fun) ret value:%d(%d %d %d %d).\r\n", 
												nResult, retDatas[1], retDatas[2], retDatas[3], retDatas[4]);///
			...
			if(nResult&(8))
			{
				strResult+=L"wifi模块通信异常\r\n";
				this->m_strJYTemp+="wifi模块通信";
				this->m_strJYTemp+="|0|0|0|Fail\r\n";
			}
			if( nResult==0)
			{
...
VOID			CLed4TestDlg::SetBtnStatus(BOOL	bEnable)
{
...
BOOL CLed4TestDlg::PreTranslateMessage(MSG* pMsg)
{
...
VOID			CLed4TestDlg::SetResultInfo()
{
	CHAR		csBuf[MAX_PATH]={0};
