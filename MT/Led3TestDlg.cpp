...
IMPLEMENT_DYNAMIC(CLed3TestDlg, CDialogEx)

CLed3TestDlg::CLed3TestDlg(LPVOID p, TESTERTYPE type, CWnd* pParent /*=NULL*/)
	: CDialogEx(CLed3TestDlg::IDD, pParent),
m_Result(MJ_NORMAL),
m_nTotalNum(0),
m_nPassNum(0),
m_nFailNum(0)
...
CLed3TestDlg::~CLed3TestDlg()
{
	CloseSerialPort(m_hDevice);
}

void CLed3TestDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CLed3TestDlg, CDialogEx)
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_BTN_START, &CLed3TestDlg::OnBnClickedBtnStart)
END_MESSAGE_MAP()
...
HBRUSH CLed3TestDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
BOOL CLed3TestDlg::OnInitDialog()
{
	...
	case TP_LED:
		{
			SetWindowTextA(this->m_hWnd, "Led-Test(sport)");
		}
		break;

	case TP_WIFI:
		{
			SetWindowTextA(this->m_hWnd, "Wifi-Test(sport)");
		}
		break;

	case TP_SPORT_MB:
		{
			SetWindowTextA(this->m_hWnd, "MainBoard-Test(sport)");
		}
		break;
	}
...
VOID			CLed3TestDlg::RefreshDeviceListR()
{
	wstring		wstr;
	VECSTR		vs;
...
VOID			CLed3TestDlg::RefreshDeviceList()
{
	wstring		wstr;
	wstring		wstrTmp;
	VECSTR		vs;
...
// 			else 
			...
			{
				bFind=TRUE;
				wstrTmp=L"(main board mode[SPORT]).\r\n";
			}
			break;
		}
...
unsigned WINAPI		_threadForCheck3(LPVOID p)
{
	CLed3TestDlg*	pThis=(CLed3TestDlg*)p;
		...
		case TP_LED:
			{
				...
				*/
				...
				pThis->m_strJYFormatFileContent+="TesterDesc|灯板测试站\r\nTesterID|0002\r\n";
		...
		case TP_SPORT_MB:
			{
				...
				pThis->m_strJYFormatFileContent+="TesterDesc|主板(SPORT)测试站\r\nTesterID|0002\r\n";
				pThis->m_strJYFormatFileContent+="PN|";
				pThis->m_strJYFormatFileContent+=pThis->m_strPSN;
				pThis->m_strJYFormatFileContent+="Line|0001\r\nOperatorNum|0002\r\nTestDate|";
				pThis->m_strJYFormatFileContent+=csBuf;
...
BOOL			CLed3TestDlg::CheckSN()
{
	...
	else if( TP_SPORT_MB==m_testType)
	{
		if( !memcmp(csBuf, "MJTW", 4) &&
			strlen(csBuf)==15 )
		{
...
void CLed3TestDlg::OnBnClickedBtnStart()
{
	this->SetBtnStatus(FALSE);
	...
	if( this->CheckSN())
	{
		...
		CloseHandle((HANDLE)_beginthreadex(NULL, NULL, _threadForCheck3, this, NULL, NULL));
	}
	else
	{
...
VOID  CLed3TestDlg::ShowInfo(PWCHAR pInfo, BOOL	bNew)
{
	wstring		wstr;
	if( bNew)
	{
...
BOOL			CLed3TestDlg::UpdaterMB(string strWorkDir)
{
	BOOL		bRet=FALSE;
...
BOOL			CLed3TestDlg::MainBoardCheck()
{
	BOOL		bRet=FALSE;
	string		strFWPath;
	string		strWorkDir;
...
//判断文件大小
//通过升级脚本"..\\FW\\sport\\MB\\" 目录内的result.txt文件的大小来判断是否成功，失败：144B    成功：1.3KB
BOOL	IsUpdateSuccess(string strFileDir)
{
	BOOL	bRet=FALSE;
	string	strFilePath	=	strFileDir;
	strFilePath			+=	"result.txt";
	CF_OutputDBGInfoEx(DBG_OUT, NULL, "file...............size.................:%s",strFilePath.c_str());
	do 
	{
		HANDLE	hFile=CreateFileA(strFilePath.c_str(), NULL, NULL, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		...
		CF_OutputDBGInfoEx(DBG_OUT, NULL, "file...............size.................:%d", GetFileSize(hFile, NULL));
		bRet = (GetFileSize(hFile, NULL)>200)? (TRUE):(FALSE);
...
BOOL			CLed3TestDlg::MainBoardSportCheck()
{
	BOOL		bRet=FALSE;
	string		strFWPath;
	string		strWorkDir;
	...
	//获取升级状态时的超时
	const	DWORD			dwTimeOutV=15*1000;
	...
	CF_GetModuleDir(csBuf);
	strFWPath=csBuf;
	strFWPath+="FW\\sport\\MB\\";
	strWorkDir=strFWPath;
	strFWPath+="airnut_sport_ble_download.bat";///
	...
	{
		//start update
		if( !P_SportMBUpdate(m_hDevice, retDatas, SHORTLEN))
		{
			this->m_strJYTemp+="进入升级模式";
			this->m_strJYTemp+="|0|0|0|Fail\r\n";

			CF_OutputDBGInfoEx(GDBGTYPE, NULL, __FUNCTION__"...entry update mode err.\r\n");
			this->ShowInfo(L"进入升级模式失败.\r\n", FALSE);
			break;
		}
		...
		while( !P_SportMBUpdateState(m_hDevice, retDatas, SHORTLEN))
		{
			//update success
			if( retDatas[0]==TP_SPORT_MB )
			{
				break;
			}

			if( GetTickCount()-dwBeginTick > dwTimeOutV )
			{
				break;
			}
			Sleep(300);
		...
		if( retDatas[0]!=TP_SPORT_MB ||
			GetTickCount()-dwBeginTick > dwTimeOutV)
		{
			this->m_strJYTemp+="获取升级状态(超时)";
			this->m_strJYTemp+="|0|0|0|Fail\r\n";

			CF_OutputDBGInfoEx(GDBGTYPE, NULL, __FUNCTION__"...get update state err(time out).\r\n");
			this->ShowInfo(L"获取升级状态失(超时)败.\r\n", FALSE);
			break;
		}


		//通过升级脚本"..\\FW\\sport\\MB\\" 目录内的result.txt文件的大小来判断是否成功，失败：144B    成功：1.3KB
		if( !IsUpdateSuccess(strWorkDir))
		{
			this->m_strJYTemp+="蓝牙下载失败";
			this->m_strJYTemp+="|0|0|0|Fail\r\n";

			CF_OutputDBGInfoEx(GDBGTYPE, NULL, __FUNCTION__"...get update state err(time out).\r\n");
			this->ShowInfo(L"蓝牙下载失败.\r\n", FALSE);
			break;
		}
		...
		if( P_GetCheckResult(m_hDevice, m_testType, retDatas, SHORTLEN))
		{
			...
			*/
			nResult=*(PLONG)(&retDatas[1]);
			nResult=ntohl(nResult);
			CF_OutputDBGInfoEx(GDBGTYPE, NULL, __FUNCTION__"...MB(sport) ret value:%d(%d %d %d %d).\r\n", 
												nResult, retDatas[1], retDatas[2], retDatas[3], retDatas[4]);///
			...
			if(nResult&(2))
			{
				strResult+=L"3.3V LDO 电源故障\r\n";
				this->m_strJYTemp+="3.3V LDO 电源";
				this->m_strJYTemp+="|0|0|0|Fail\r\n";
			}
			if(nResult&(4))
			{
				strResult+=L"电池电压异常\r\n";
				this->m_strJYTemp+="电池电压";
				this->m_strJYTemp+="|0|0|0|Fail\r\n";
			}
			if(nResult&(8))
			{
				strResult+=L"蓝牙内部LDO电源异常\r\n";
				this->m_strJYTemp+="蓝牙内部LDO电源";
				this->m_strJYTemp+="|0|0|0|Fail\r\n";
			}
			if(nResult&(16))
			{
				strResult+=L"蓝牙芯片异常\r\n";
				this->m_strJYTemp+="蓝牙芯片";
				this->m_strJYTemp+="|0|0|0|Fail\r\n";
			}
			if(nResult&(32))
			{
				strResult+=L"RTC时钟异常\r\n";
				this->m_strJYTemp+="RTC时钟";
				this->m_strJYTemp+="|0|0|0|Fail\r\n";
			}
			if( nResult==0)
			{
...
VOID			CLed3TestDlg::SetBtnStatus(BOOL	bEnable)
{
...
BOOL CLed3TestDlg::PreTranslateMessage(MSG* pMsg)
{
...
VOID			CLed3TestDlg::SetResultInfo()
{
	CHAR		csBuf[MAX_PATH]={0};
