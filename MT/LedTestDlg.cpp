...
IMPLEMENT_DYNAMIC(CLedTestDlg, CDialogEx)

CLedTestDlg::CLedTestDlg(LPVOID p, TESTERTYPE type, CWnd* pParent /*=NULL*/)
	: CDialogEx(CLedTestDlg::IDD, pParent),
m_Result(MJ_NORMAL),
m_nTotalNum(0),
m_nPassNum(0),
m_nFailNum(0)
...
CLedTestDlg::~CLedTestDlg()
{
	CloseSerialPort(m_hDevice);
}

void CLedTestDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CLedTestDlg, CDialogEx)
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_BTN_START, &CLedTestDlg::OnBnClickedBtnStart)
END_MESSAGE_MAP()
...
HBRUSH CLedTestDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
BOOL CLedTestDlg::OnInitDialog()
{
	...
	case TP_LED:
		{
			SetWindowTextA(this->m_hWnd, "Led-Test(1s)");
		}
		break;

	case TP_WIFI:
		{
			SetWindowTextA(this->m_hWnd, "Wifi-Test(1s)");
		}
		break;

	case TP_MB:
		{
			SetWindowTextA(this->m_hWnd, "MainBoard-Test(1s)");
		}
		break;
	}
...
VOID			CLedTestDlg::RefreshDeviceListR()
{
	wstring		wstr;
	VECSTR		vs;
...
VOID			CLedTestDlg::RefreshDeviceList()
{
	wstring		wstr;
	wstring		wstrTmp;
	VECSTR		vs;
...
unsigned WINAPI		_threadForCheck(LPVOID p)
{
	CLedTestDlg*	pThis=(CLedTestDlg*)p;
	...
	if( pThis)
	{
		...
		OutputDebugStringA(pThis->m_strJYFormatFileContent.c_str());
		CFileHelper::_()->SaveLogInfo(pThis->m_testType, (PCHAR)pThis->m_strFileContent.c_str(), pThis->m_strFileContent.length()+1);
		CFileHelper::_()->SaveJYLogInfo(pThis->m_testType, AIRUS1S, (PCHAR)pThis->m_strJYFormatFileContent.c_str(), pThis->m_strJYFormatFileContent.length()+1);
	}
...
void CLedTestDlg::OnBnClickedBtnStart()
{
	this->SetBtnStatus(FALSE);
...
VOID  CLedTestDlg::ShowInfo(PWCHAR pInfo, BOOL	bNew)
{
	wstring		wstr;
	if( bNew)
	{
...
BOOL			CLedTestDlg::UpdaterMB(string strWorkDir)
{
	BOOL		bRet=FALSE;
...
BOOL			CLedTestDlg::MainBoardCheck()
{
	BOOL		bRet=FALSE;
	string		strFWPath;
	string		strWorkDir;
...
VOID			CLedTestDlg::SetBtnStatus(BOOL	bEnable)
{
...
VOID			CLedTestDlg::SetResultInfo()
{
	CHAR		csBuf[MAX_PATH]={0};
